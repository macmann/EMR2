...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const diag = await prisma.diagnosis.create({
      data: { visitId: id, diagnosis: parsed.data.diagnosis },
    });
    await logDataChange(
      req.user!.userId,
      "diagnosis",
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (q) {
      ...
      SELECT diagId, visitId, diagnosis, createdAt
      FROM Diagnosis
      WHERE LOWER(diagnosis) LIKE LOWER(${`%${q}%`})
    ...
    // No text search - use Prisma for date filtering
    const where: any = {};
    if (from || to) {
      where.createdAt = {
        ...(from && { gte: from }),
